...
import { MaternityImg } from "../../../services/ImagesServices";
...
function Maternity() {
      ...
      .then((response) => {
        ...
        categorizeImages(images)
          .then(({ largeImages, smallImages }) => {
            ...
            setMaternityImage(arrangedImages);
            setSpinner(false);
            setTimeout(() => {
  ...
  return (
    <div className="px-2 flex flex-col justify-center pt-20 lg:pt-12 items-center bg-[#F7F9F2]">
      <ScrollToTop />

      {CommonHeading("Maternity Moments")}
      ...
      {maternityImage.length > 0 && !spinner ? (
        <div ref={galleryRef}>
          <LightGallery
            speed={500}
            ...
            mode="lg-fade"
          >
            {maternityImage.map((image, index) => (
