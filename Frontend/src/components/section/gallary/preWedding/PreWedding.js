...
import { PreWeddingImg } from "../../../services/ImagesServices";
...
function PreWedding() {
      ...
      .then((response) => {
        ...
        categorizeImages(images)
          .then(({ largeImages, smallImages }) => {
            ...
            setPreWeddingImage(arrangedImages);
            setSpinner(false);
            setTimeout(() => {
  ...
  return (
    <div className="px-2 flex flex-col justify-center  pt-20 lg:pt-12 items-center bg-slate-50">
      ...
      {CommonHeading("Pre-Wedding Magic")}
      ...
      {preWeddingImage.length > 0 && !spinner ? (
        <div ref={galleryRef}>
          <LightGallery
            speed={500}
            ...
            mode="lg-fade"
          >
            {preWeddingImage.map((image, index) => (
