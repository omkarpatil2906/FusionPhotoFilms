...
import { EngagementImg } from "../../../services/ImagesServices";
...
function Engagement() {
      ...
      .then((response) => {
        ...
        categorizeImages(images)
          .then(({ largeImages, smallImages }) => {
            ...
            setEngagementImage(arrangedImages);
            setSpinner(false);
            setTimeout(() => {
  ...
  return (
    <div className="px-2 flex flex-col justify-center pt-20 lg:pt-12 items-center bg-slate-50">
      <ScrollToTop />
      {CommonHeading("Engagement Highlights")}
      {spinner && (
      ...
      {engagementImage.length > 0 && !spinner ? (
        <div ref={galleryRef}>
          <LightGallery
            speed={500}
            ...
            mode="lg-fade"
          >
            {engagementImage.map((image, index) => (
